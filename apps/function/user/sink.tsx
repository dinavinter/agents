    ...
    (c) =>
        c.render(
            <div
                ...
                sse-swap="account"
                hx-swap="beforeend transitions:true swap:1s settle:1s scroll:bottom"
        ...
        for await (const [key, {
            ...
            await stream.writeSSE({
                ...
                data: `<pre class="shadow p-2 text-sm bg-slate-100 border-2 border-slate-200 transition-all">
                            ${label} <span sse-swap="@label.${key}"  hx-swap="textContent transision:true">
                                <span  hx-get="${key}/status" hx-trigger="intersect once" hx-swap="outerHTML transition:true swap:1s settle:1s"></span>
...
app.get("/:uid/status", async (c) => {
    ...
    return c.html(<span class="text-slate-900" hx-get={c.req.url} hx-trigger={trigger}
                        hx-swap="outerHTML transition:true swap:1s settle:1s">!!{status}</span>)
})
...
//
// <!--
//     <span sse-swap="@label.${key}"  hx-swap="textContent transision:true"></span>
< span
hx - get = "${key}/status"
hx - trigger = "intersect once"
// hx - swap = "outerHTML transition:true swap:1s settle:1s" > < /span>
// <div hx-get="https://avatar-gen.deno.dev/${key}" hx-trigger="load delay:1s" hx-swap="innerHTML transition:true swap:1s settle:1s"></div>
// -->