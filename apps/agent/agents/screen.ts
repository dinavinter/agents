    ...
    states: {
        ...
        draft: {
            entry: renderTo('content', ({stream, html}) => html`
                        <div class="flex items-start gap-2.5  p-2 m-2 w-full">
                            <img class="w-12 h-12 rounded-full"
                                 src="https://flowbite.com/docs/images/people/profile-picture-5.jpg" alt="thinker avatar"/>
                            <div class="flex flex-col gap-1 w-full ">
                                <div class="flex items center space-x-2 rtl:space-x-reverse">
                                    <span class="sm:text-sm md:text-lg lg:text-2xl font-semibold text-gray-900 dark:text-white">Screen Set Assistant</span>
                                    <span class="text-sm  lg:text-lg font-normal text-gray-500 dark:text-gray-400">${new Date(Date.now()).toLocaleTimeString()}</span>
                                </div>
                                <div class="leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 flex-grow ">
                                    <div class="text-lg text-slate-900 inline text-wrap" sse-swap="screens"/>
                                </div>
                            </div>
                        </div>
                `
                ...
                '*': {
                    actions: renderTo('screens', ({html, event: {name, description}}) => html`
                        <div class="flex items start gap-2.5  p-2 m-2 w-full">
                            ...
                            <form shadowDom  id="${name}" hx-ext="sse" sse-swap="@screen.${name}" hx-swap="beforeend" class="shadow-lg border-slate-400 ">
                                    <!-- ${description} -->
                                <pre class="w-2/3 text-wrap text-green-500 overflow-ellipsis">&lt;!--${description}--&gt;</pre>
                ...
                input: ({context:{draft}})=>({
                    template: `You are an expert in Gigya schema and html fields, your task is to help add the missing fields in the form drafts, go over the containers comments and instructions, and publish html fields to each screen  your code will be swapped into the appropriate screen
                     ...
                     schema: z.object({
                        ...
                        outerHTML: z.string().describe('the field in a valid html.'),
        ...
        css: {
            ...
            on: {
                ...
                '*': {
                    actions: emit(({event: {type, data, event}}) => ({
                        event: `@css.${event}`,
                        type: 'css',
                        data: data
                    }))
