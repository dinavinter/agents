...
//NOTE: works with assistant api of azure-ai / openai, won't work with sap ai
...
const setupAssistant = async () => {
    try {
        const assistantResponse = await assistantsClient.beta.assistants.create({
            ...
            name: "tester",
            instructions: "Your task is to generate api tests for the given request, provide code only respoonse, search for similar tests to understand how to use gigya apis, answer only in c# code, with no additional comments",
...
export const machine =  setup({
    ...
    actors:{
        ...
        runStream: fromPromise(async ({input:{threadId}}:{input: {threadId:string}}) => {
            return assistantsClient.beta.threads.runs.stream(threadId, {
                assistant_id: env.ASSISTANT_ID!
            });
        }),
        streamThread: fromEventAsyncGenerator(async function * ({input:{threadId}, system, emit}:{input: {threadId:string}, system: ActorSystem<any>, emit: (emitted: AssistantEmittedEventObject) => void}) {
            const stream = assistantsClient.beta.threads.runs.stream(threadId, {
                assistant_id: env.ASSISTANT_ID!
            });
    ...
    states: {
        init: {
            entry: render(({html}) => {
                return html`
                    <main class="mx-auto  bg-slate-50 h-full">
                        ...
                        <div class="flex flex-col items-center justify-center *:w-full w-full *:justify-center" hx-ext="sse"
            ...
            invoke: {
                src: 'createThread',
                input: ({context: {createParams}}) => createParams,
                onDone: {
                    target: 'thread',
                    actions: assign({
                    ...
                    <div class="flex items start gap-2.5  p-2 m-2 w-full">
                        ...
                        <div class="flex flex-col gap-1 w-full ">
                            ...
                            <div class="leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 flex-grow " >
                                <form>
                                   <input type="text" autocomplete="true" list="examples" class="w-full p-2 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700 flex-grow " name="message" placeholder="Type a message"   />
        ...
        message: {
            invoke: {
                src: 'sendMessage',
                input: ({context: {thread, message}}) => ({thread, message: message}),
                onDone: {
                    target: 'stream',
                    actions: assign({
        ...
        stream: {
            ...
            invoke: {
                src: 'streamThread',
                input: ({context: {thread}}) => ({threadId: thread}),
                id: 'assistant',
                onDone: 'thread'
            },
            on: {
                ...
                ["thread.message.created"]: {
                    actions: renderTo('messages', ({html, event}) =>
                        html`<pre class="text-pretty language-csharp"><code class="font-mono text-slate-900 inline text-wrap" hx-ext="sse" sse-swap=${`@${event.id}.text-delta`} /> </pre>`
                    )
                }
