...
export function Debugger({docId}: { docId: string }) {
    const doc: Y.Doc = useYDoc()
    const awareness = useAwareness()

    return (
        <div>
            <h2 className="text-3xl font-bold mb-2"> {docId}</h2>
            <p className="text-gray-300 mb-4">
                {doc.guid} / {doc.collectionid}
            </p>
            ...
            <code className={"prettyprint"}>{JSON.stringify(doc.toJSON(), null, 2)}</code>
           </pre>
                    <div className="flex-1 container ">
                        <h2 className="text-2xl font-bold mb-4">Subdocs</h2>
                        <div className={"*:m-3 *:p-4"}>{Array.from(doc.subdocs).map((doc) => (
                           <SubdocView  doc={doc} /> 
                        ))}</div>
...
function SubdocView({doc}: {doc:Y.Doc}) {
    return (<details>
            <summary>
            ...
            <div className="flex-1">
               ...
               <code className={"prettyprint"}>{JSON.stringify(doc.toJSON(), null, 2)}</code>
                ...
                <div className="flex-1">
                    <h2 className="text-2xl font-bold mb-4">Subdocs</h2>
                    {/*<div className={"*:m-3 *:p-4"}>{Array.from(doc.subdocs).map((doc) => (*/}
                    {/*    <SubdocView key={doc.guid} doc={doc}/>*/}
                    {/*))}</div>*/}
                </div>
            </div>
        </details>
