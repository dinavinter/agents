...
type DeploymentMachineEvent =
    | { type: "CONFIGURE" } & Projects.DeploymentCreateParams
    | { type: "ASSET"; name: string; file: File | Symlink }
    | { type: "START" } & Projects.DeploymentCreateParams
    | { type: "REDEPLOY" } & Deployments.DeploymentRedeployParams
    | { type: "STOP" }
    | { type: "STOPPED" }
...
export const deploymentMachine = setup({
    delays: {
        DEPLOYMENT_POLL_INTERVAL: 1000,
        BUILD_LOGS_INTERVAL: 1000,
        APP_LOGS_INTERVAL: 1000,
    ...
    actors: {
        ...
        buildLogsObserver: fromEventAsyncGenerator(async function* ({ input: { interval, deployment, logs: previousLogs } }: { input: { interval: number, deployment: string, logs: { message: string, level: string }[] } }) {
        ...
        appLogs: fromEventAsyncGenerator(async function* ({ input: { deployment, logs, interval } }: { input: { interval: number, deployment: string, logs: { time: string }[] } }) {
            ...
            while (true) {
                ...
                for (const log of logs.filter(l => logs.includes(l))) {
                    yield {
                        type: `log.${log.level}`,
                        message: log.message,
                        time: log.time,
        ...
        buildLogs: fromCallback(function({ receive, sendBack, input }) {
            subhosting.deployments.buildLogs.get(input).then((res) => {
            ...
            receive(async ({ type }) => {
                console.log("build logs received", input, type)
                if (type === "PING") {
                    const res = await subhosting.deployments.buildLogs.get(input);
                    console.log("build logs response", res)
                    for (const log of res) {
                        sendBack({
                            type: `log.${log.level}`,
                            message: log.message
    ...
    guards: {
        canRetry: ({ context }) => {
            // Implement retry logic based on error type or count
    ...
    states: {
        ...
        pending: {
            ...
            states: {
                polling: {
                  after: {
                    DEPLOYMENT_POLL_INTERVAL: { actions: sendTo('deployment' , { type: 'PING' }) },
                    BUILD_LOGS_INTERVAL: { actions: sendTo('build' , { type: 'PING' }) },
                  }
                }
              }
        },
